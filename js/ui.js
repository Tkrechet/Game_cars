...
function updateProgressBar() {
    ...
    if (GameState.currentLevel === 2) {
        ...
        // Используем completedLaps + 1 для текущего круга
        // completedLaps увеличивается только после выхода из финишной зоны
        // Если машина в финишной зоне на последнем круге (completedLaps = laps - 1), показываем последний круг
...
function setupEventHandlers() {
    ...
    // Обработчик нажатия Enter для кнопки "Начать"
    window.addEventListener('keydown', (event) => {
        // Проверяем, что нажата клавиша Enter
        if (event.key === 'Enter' || event.keyCode === 13) {
            const startBtn = document.getElementById('startLevel3Btn');
            // Проверяем, что кнопка существует, не заблокирована и видна
            if (startBtn && !startBtn.disabled && startBtn.offsetParent !== null) {
                // Проверяем, что пользователь не находится в поле ввода или другом интерактивном элементе
